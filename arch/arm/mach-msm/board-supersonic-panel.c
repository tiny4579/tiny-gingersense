				...
				enum led_brightness val)
{
	...
	if (test_bit(GATE_ON, &cabc.status) == 0){
		printk(KERN_DEBUG "suc_set_brightness(): set brightness = %d\n", val);
		return;
	}
	...
	if (panel_type == PANEL_SHARP) {
		...
		for (i = 0; i < ARRAY_SIZE(pwm_seq); i++) {
			reg = pwm_seq[i].reg;
			sleep_time = pwm_seq[i].val;
			if (reg == REG_WAIT)
				hr_msleep(sleep_time);
			else
				client->remote_write(client, cpu_to_le32(sleep_time), reg);
		}
		client->remote_write(client, shrink_br, 0x14B4);
	} else {
		printk(KERN_DEBUG "PANEL_AUO: set brightness = %d\n", val);
...
#define	SUPERSONIC_GPIO_SPI_CLK	17
#define	SUPERSONIC_GPIO_SPI_DO	18
#define	SUPERSONIC_GPIO_SPI_DI	19
#define	SUPERSONIC_GPIO_SPI_CS	20

#define LCM_GPIO_CFG(gpio, func) \
PCOM_GPIO_CFG(gpio, func, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_16MA)
static uint32_t spi_on_gpio_table[] = {
	LCM_GPIO_CFG(SUPERSONIC_GPIO_SPI_CLK, 1),
	LCM_GPIO_CFG(SUPERSONIC_GPIO_SPI_CS, 1),
	LCM_GPIO_CFG(SUPERSONIC_GPIO_SPI_DO, 1),
	PCOM_GPIO_CFG(SUPERSONIC_GPIO_SPI_DI, 1, GPIO_INPUT, GPIO_NO_PULL, GPIO_16MA),
};

static uint32_t spi_off_gpio_table[] = {
	LCM_GPIO_CFG(SUPERSONIC_GPIO_SPI_CLK, 0),
	LCM_GPIO_CFG(SUPERSONIC_GPIO_SPI_CS, 0),
	LCM_GPIO_CFG(SUPERSONIC_GPIO_SPI_DO, 0),
	PCOM_GPIO_CFG(SUPERSONIC_GPIO_SPI_DI, 0, GPIO_INPUT, GPIO_NO_PULL, GPIO_16MA),
};
