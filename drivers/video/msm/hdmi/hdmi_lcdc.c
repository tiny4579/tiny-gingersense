...
struct mdp_hdmi_lcdc_info *_lcdc;

static struct mdp_device *mdp_dev;

#define panel_to_hdmi_lcdc(p) container_of((p), struct mdp_hdmi_lcdc_info, fb_panel_data)
...
{
        struct mdp_hdmi_lcdc_info *lcdc = _lcdc;
...
{
	struct mdp_hdmi_lcdc_info *lcdc = panel_to_hdmi_lcdc(fb_panel);
	struct msm_lcdc_panel_ops *panel_ops = lcdc->pdata->panel_ops;
		...
		struct msm_lcdc_timing *timing, u32 xres, u32 yres)
{
	...
	HDMI_DBG("%s, clk=%d, xres=%d, yres=%d,\n", __func__,
		(unsigned int)clk_get_rate(lcdc->pclk), xres, yres);
...
static int lcdc_hw_init(struct mdp_hdmi_lcdc_info *lcdc)
{
...
{
	struct mdp_hdmi_lcdc_info *lcdc = panel_to_hdmi_lcdc(panel);
	int ret;
...
{
	struct mdp_hdmi_lcdc_info *lcdc;

	lcdc = container_of(cb, struct mdp_hdmi_lcdc_info, frame_start_cb);
...
static void lcdc_dma_start(void *priv, uint32_t addr, uint32_t stride,
			   ...
			   uint32_t y)
{
	struct mdp_hdmi_lcdc_info *lcdc = priv;
...
static void precompute_timing_parms(struct mdp_hdmi_lcdc_info *lcdc)
{
...
static int hdmi_lcdc_probe(struct platform_device *pdev)
{
	...
	_lcdc = lcdc = kzalloc(sizeof(struct mdp_hdmi_lcdc_info), GFP_KERNEL);
	if (!lcdc)
		return -ENOMEM;
...
{
	struct mdp_hdmi_lcdc_info *lcdc = platform_get_drvdata(pdev);
